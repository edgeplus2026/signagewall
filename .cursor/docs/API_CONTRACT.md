...
| GET | `/playlists` | Bearer + `X-Organization-Id` | List summaries |
| POST | `/playlists` | Bearer + `X-Organization-Id` | Create |
| GET | `/playlists/:id` | Bearer + `X-Organization-Id` | Metadata |
| PATCH | `/playlists/:id` | Bearer + `X-Organization-Id` | Update name/description |
| POST | `/playlists/delete` | Bearer + `X-Organization-Id` | Bulk delete `{ ids }` |
| POST | `/playlists/:id/duplicate` | Bearer + `X-Organization-Id` | Clone playlist + items |
| GET | `/playlists/:id/items` | Bearer + `X-Organization-Id` | List items |
| PUT | `/playlists/:id/items` | Bearer + `X-Organization-Id` | Replace all items |
| DELETE | `/playlists/:id/items` | Bearer + `X-Organization-Id` | Bulk remove items `{ ids }` |
...
Used by: `/members/*`, `/settings/feedback`, `/settings/report-problem`, `/media/*`, `/playlists/*`, and future domain modules (screens).
...
## Playlists (`edge-fe/src/features/playlists`)

Org-scoped. Requires `X-Organization-Id`. List/search is client-side (no pagination). Frontend query keys include `organizationId` so cache does not leak across org switches.

| Method | Path | Request body | Response `data` |
|--------|------|--------------|-----------------|
| GET | `/playlists` | — | `PlaylistSummary[]` |
| POST | `/playlists` | `{ name, description? }` | `PlaylistDetail` |
| GET | `/playlists/:id` | — | `PlaylistDetail` |
| PATCH | `/playlists/:id` | `{ name?, description? }` | `PlaylistDetail` |
| POST | `/playlists/delete` | `{ ids: string[] }` | `null` |
| POST | `/playlists/:id/duplicate` | `{ copySuffix?: string }` | `PlaylistDetail` |
| GET | `/playlists/:id/items` | — | `PlaylistItem[]` |
| PUT | `/playlists/:id/items` | `{ items: ReplacePlaylistItem[] }` | `PlaylistItem[]` |
| DELETE | `/playlists/:id/items` | `{ ids: string[] }` | `null` |
...
`thumbnailUrl` is resolved at read time from the first item's media (`coverMediaId` on the document). `totalDuration` is the sum of **non-disabled** item durations and is recomputed when items change.

**Validation:** media must exist in the org, be image/video, and have `status === 'ready'`. `PATCH` requires at least one of `name` or `description`. Item `id` values in a replace payload must be unique.
