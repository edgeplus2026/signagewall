...
- [x] **BE-5b: Tenancy e2e pack** — `apps/be/test/tenancy.e2e-spec.ts` against the real AppModule on an in-memory Mongo replica set (`pnpm --filter @signagewall/be test:e2e`): cross-org reads/writes 403/404, org-header forgery, viewer read-only ceiling, recovery-link scoping, plan limit fail-closed, verification gate, refresh rotation + replay rejection, password-change revocation, lockout. Writing it exposed two real bugs, both fixed: `refreshTokenHash`/reset-token clears used `undefined` in Mongoose updates (silently dropped — sessions survived password reset, reset links were reusable), and refresh hashes used bcrypt whose 72-byte truncation made every one of a user's refresh tokens match every other (rotation revoked nothing; now SHA-256 + timing-safe compare).
