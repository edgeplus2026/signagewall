...
describe('AvailabilityEvaluator', () => {
  ...
  describe('always-on', () => {
    it('is always on and never transitions', () => {
      ...
      expect(
        evaluator.nextTransition(a, utc('2026-06-01T03:00:00Z')),
      ).toBeNull();
    });

    it('treats missing availability as always-on', () => {
      expect(evaluator.isOnAt(undefined, utc('2026-06-01T03:00:00Z'))).toBe(
        true,
      );
      expect(
        evaluator.nextTransition(undefined, utc('2026-06-01T03:00:00Z')),
      ).toBeNull();
    });
  });
  ...
  describe('weekly', () => {
    ...
    const monOnly = weeklyAvailability({
      [WeekdayKey.MONDAY]: { start: '09:00', end: '17:00' },
    });
    ...
    it('is on inside the window and off outside it', () => {
      ...
      expect(evaluator.isOnAt(monOnly, utc('2026-06-01T06:00:00Z'))).toBe(
        false,
      ); // 08:00 local
      expect(evaluator.isOnAt(monOnly, utc('2026-06-01T16:00:00Z'))).toBe(
        false,
      ); // 18:00 local
    });

    it('is off all day on disabled days', () => {
      expect(evaluator.isOnAt(monOnly, utc('2026-06-02T08:00:00Z'))).toBe(
        false,
      ); // Tuesday
      expect(evaluator.isOnAt(monOnly, utc('2026-06-02T12:00:00Z'))).toBe(
        false,
      );
    });
    ...
    it('is off when no days are enabled', () => {
      ...
      expect(
        evaluator.nextTransition(none, utc('2026-06-01T08:00:00Z')),
      ).toBeNull();
    });
    ...
    it('produces no window for a reversed (would-be overnight) range', () => {
      ...
      expect(evaluator.isOnAt(reversed, utc('2026-06-05T20:00:00Z'))).toBe(
        false,
      );
      expect(evaluator.isOnAt(reversed, utc('2026-06-05T23:30:00Z'))).toBe(
        false,
      );
    });
    ...
    it('reports a single clean transition for a long same-day window', () => {
      ...
      expect(
        evaluator.nextTransition(monLong, utc('2026-06-01T10:00:00Z')),
      ).toEqual({
    ...
    it('rejects a non-existent local start time on a DST spring-forward day', () => {
      ...
      expect(evaluator.isOnAt(sundayGap, utc('2026-03-29T02:00:00Z'))).toBe(
        false,
      );
    });
    ...
    it('preserves wall-clock time across DST (winter offset differs from summer)', () => {
      ...
      expect(evaluator.isOnAt(monOnly, utc('2026-01-05T07:30:00Z'))).toBe(
        false,
      ); // 08:30 local
    });
  });
  ...
  describe('special', () => {
    ...
    it('has no further transition once the range has passed', () => {
      expect(
        evaluator.nextTransition(range, utc('2026-06-13T08:00:00Z')),
      ).toBeNull();
    });
    ...
    it('produces no window for a reversed time range', () => {
      ...
      expect(evaluator.isOnAt(reversed, utc('2026-06-10T23:00:00Z'))).toBe(
        false,
      );
    });
  });
...
describe('validateAvailability', () => {
  ...
  it('rejects an enabled day where start is not before end (equal)', () => {
    const a = weeklyAvailability({
      [WeekdayKey.MONDAY]: { start: '09:00', end: '09:00' },
    });
    expect(validateAvailability(a)).toBe(
      AvailabilityValidationError.INVALID_TIME_RANGE,
    );
  });

  it('rejects an enabled day where start is after end (reversed)', () => {
    const a = weeklyAvailability({
      [WeekdayKey.MONDAY]: { start: '17:00', end: '09:00' },
    });
    expect(validateAvailability(a)).toBe(
      AvailabilityValidationError.INVALID_TIME_RANGE,
    );
  ...
  it('rejects an invalid timezone', () => {
    ...
    expect(validateAvailability(a)).toBe(
      AvailabilityValidationError.INVALID_TIMEZONE,
    );
  });
  ...
  it('rejects a special range where end date is before start date', () => {
    ...
    expect(validateAvailability(a)).toBe(
      AvailabilityValidationError.INVALID_DATE_RANGE,
    );
  });
  ...
  it('rejects a special block with an impossible calendar date', () => {
    ...
    expect(validateAvailability(a)).toBe(
      AvailabilityValidationError.INVALID_DATE,
    );
  });
  ...
  it('rejects an incomplete weekly list', () => {
    ...
    expect(validateAvailability(a)).toBe(
      AvailabilityValidationError.INCOMPLETE_WEEKLY,
    );
  });
});
