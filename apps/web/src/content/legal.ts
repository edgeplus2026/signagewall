 ...
 *
 * Moved here from `apps/be/src/modules/legal/legal.content.ts`, where they were
 * served to the dashboard over an API and never had a public URL. The website
 * is now the single place the text lives and the only place it is published;
 * the backend keeps the version number and the record of who accepted what, and
 * links here.
 *
 * Company details interpolate from `COMPANY` rather than being typed in, so the
 * registered name and address cannot drift between three documents.
 *
 * Markdown, in a deliberately small subset — `#`, `##`, `-` bullets and
 * `_emphasis_`. See `legal-markdown.tsx`, which renders exactly that and
 * nothing else.
 */
...
## 19. Contact
Questions about these Terms: ${COMPANY.email}. ${COMPANY.legalName}, ${COMPANY.address}.`
...
## 19. Kontakt
Pitanja o ovim Uslovima: ${COMPANY.email}. ${COMPANY.legalName}, ${COMPANY.address}.`
...
controller is ${COMPANY.legalName}, ${COMPANY.address}, matični broj ${COMPANY.registrationNumber}, PIB ${COMPANY.taxNumber}. We process data
...
${COMPANY.email}. We will respond within the timeframes required by law.
...
## 13. Contact
Privacy questions or requests: ${COMPANY.email}. ${COMPANY.legalName}, ${COMPANY.address}. If we have appointed a data protection officer, their contact
...
Rukovalac podacima je ${COMPANY.legalName}, ${COMPANY.address}, matični broj ${COMPANY.registrationNumber}, PIB ${COMPANY.taxNumber}. Podatke
...
nas na ${COMPANY.email}. Odgovorićemo u rokovima koje propisuje zakon.
...
## 13. Kontakt
Pitanja ili zahtevi u vezi sa privatnošću: ${COMPANY.email}. ${COMPANY.legalName}, ${COMPANY.address}. Ako smo imenovali lice za zaštitu podataka, njegovi kontakt
...
signage dashboard and the player application are covered by the Privacy Policy.

## 1. What we store
We keep the list deliberately short. Everything below is either needed for the
site to work or is switched on only after you agree to it.

- \`NEXT_LOCALE\` — a cookie holding your language choice (\`sr\` or \`en\`), so the
  site opens in the same language next time. Essential; set on every visit.
- \`signagewall-cookie-consent\` — a browser storage entry recording whether you accepted
  or declined analytics, so you are not asked on every page. Essential.
- \`theme\` — a browser storage entry holding your light or dark preference.
  Essential.
- \`_ga\` and \`_ga_<id>\` — Google Analytics cookies, used to count visits and see
  which pages are read. **Set only after you accept.** If you decline, or ignore
  the banner, Analytics is never loaded and these are never created.

## 2. What we do not do
We run no advertising, remarketing or cross-site tracking cookies, and we do not
sell or share what we collect with data brokers.

## 3. Analytics, and your consent
Analytics stays off until you press Accept. Declining costs you nothing — every
part of the site works the same either way. You can change your mind at any time
by clearing this site's data in your browser, which removes the stored choice
and brings the banner back.

## 4. Managing cookies in your browser
Every major browser lets you view, block and delete cookies and site storage,
usually under Settings → Privacy. Blocking the essential entries above will not
break the site, but it will forget your language and theme on each visit.
...
kontrolnu tablu i plejer pokriva Politika privatnosti.

## 1. Šta čuvamo
Lista je namerno kratka. Sve navedeno je ili neophodno da sajt radi, ili se
uključuje tek pošto pristanete.

- \`NEXT_LOCALE\` — kolačić sa izborom jezika (\`sr\` ili \`en\`), da bi se sajt
  sledeći put otvorio na istom jeziku. Neophodan; postavlja se pri svakoj poseti.
- \`signagewall-cookie-consent\` — zapis u memoriji pregledača o tome da li ste prihvatili
  ili odbili analitiku, da vas ne pitamo na svakoj stranici. Neophodan.
- \`theme\` — zapis u memoriji pregledača sa izborom svetle ili tamne teme.
  Neophodan.
- \`_ga\` i \`_ga_<id>\` — kolačići Google Analytics-a, za brojanje poseta i uvid u
  to koje se stranice čitaju. **Postavljaju se tek pošto prihvatite.** Ako
  odbijete, ili ne dirate traku, Analytics se nikada ne učitava i ovi kolačići
  ne nastaju.

## 2. Šta ne radimo
Ne koristimo kolačiće za oglašavanje, remarketing ni praćenje kroz druge sajtove,
i ne prodajemo niti delimo prikupljeno sa posrednicima podataka.

## 3. Analitika i vaš pristanak
Analitika je isključena dok ne pritisnete Prihvatam. Odbijanje vas ništa ne
košta — sajt radi isto u oba slučaja. Predomisliti se možete kad god, brisanjem
podataka ovog sajta u pregledaču: time se briše sačuvani izbor i traka se
ponovo pojavljuje.

## 4. Upravljanje kolačićima u pregledaču
Svaki veći pregledač omogućava pregled, blokiranje i brisanje kolačića i
podataka sajta, obično pod Podešavanja → Privatnost. Blokiranje neophodnih
zapisa neće pokvariti sajt, ali će vaš jezik i tema biti zaboravljeni pri svakoj
poseti.
...
export function legalDocument(doc: LegalKey, locale: string): string {
  return LEGAL_DOCUMENTS[doc][locale === 'en' ? 'en' : 'sr']
}
