import { useTranslation } from "react-i18next"
import { toast } from "sonner"

import { useImportFromDrive } from "@/features/media/hooks/useMedia"
import {
  mediaActionCardClassName,
  mediaActionCardIconClassName,
} from "@/features/media/lib/mediaActionCardStyles"
import { cn } from "@/lib/utils"

const MOCK_DRIVE_FILES = [
  {
    name: "summer-campaign.jpg",
    type: "image" as const,
    size: 1_024_000,
    width: 1920,
    height: 1080,
  },
  {
    name: "product-showcase.mp4",
    type: "video" as const,
    size: 8_388_608,
    width: 1280,
    height: 720,
  },
]

function GoogleDriveIcon({ className }: { className?: string }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      className={className}
    >
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
        fill="#4285F4"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        fill="#FBBC05"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill="#EA4335"
      />
    </svg>
  )
}

interface GoogleDriveCardProps {
  parentId: string | null
  disabled?: boolean
}

export function GoogleDriveCard({ parentId, disabled }: GoogleDriveCardProps) {
  const { t } = useTranslation()
  const importFromDrive = useImportFromDrive()
  const isDisabled = (disabled ?? false) || importFromDrive.isPending

  const handleClick = async () => {
    try {
      await importFromDrive.mutateAsync({
        parentId,
        files: MOCK_DRIVE_FILES,
      })
      toast.success(t("media.googleDrive.success"))
    } catch {
      toast.error(t("media.googleDrive.error"))
    }
  }

  return (
    <button
      type="button"
      disabled={isDisabled}
      onClick={() => void handleClick()}
      className={cn(
        mediaActionCardClassName,
        "border border-secondary bg-panel hover:border-brand/50 hover:bg-highlight/30"
      )}
    >
      <div className={mediaActionCardIconClassName}>
        <GoogleDriveIcon className="size-4" />
      </div>
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium text-primary">
          {t("media.googleDrive.title")}
        </p>
        <p className="line-clamp-2 text-xs text-secondary">
          {t("media.googleDrive.hint")}
        </p>
      </div>
    </button>
  )
}
