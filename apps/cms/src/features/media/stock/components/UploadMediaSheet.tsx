...
export function UploadMediaSheet({
  ...
  const handleUpload = () => {
    ...
    const localFiles = valid.filter((item) => item.kind === 'local').map((item) => item.file)
  ...
  return (
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent
        className={cn(
          'max-h-[85vh]',
          tab === 'stock' || validation.validItems.length ? 'h-[85vh]' : '',
        )}
      >
        <div
          className={cn(
            'mx-auto flex min-h-0 flex-1 flex-col gap-4',
            tab === 'device' ? 'max-w-4xl' : 'max-w-full',
          )}
        >
          <DrawerHeader>
            <DrawerTitle>{t('media.upload.sheet.title')}</DrawerTitle>
            <DrawerDescription>{t('media.upload.sheet.description')}</DrawerDescription>
          </DrawerHeader>
          ...
          >
            <TabsList className="mr-auto">
              <TabsTrigger value="device">{t('media.upload.sheet.deviceTab')}</TabsTrigger>
              <TabsTrigger value="stock">{t('media.upload.sheet.stockTab')}</TabsTrigger>
            </TabsList>

            <TabsContent value="device" className="min-h-0 flex-1 pb-5">
              <MyDeviceTab />
            </TabsContent>
          ...
          {showFooter ? (
            <DrawerFooter>
              ...
              <div className="flex items-center justify-end gap-2">
                <Button type="button" variant="ghost" size="sm" onClick={clearStaging}>
