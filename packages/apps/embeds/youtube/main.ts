...
/**
 * Latest state from the host. We only mount the (autoplaying) YouTube iframe
 * once we are *active* — the player preloads the next item into a hidden slot,
 * and a `display:none` iframe still plays audio, so mounting on config alone
...
function render(): void {
  ...
  const embed = active ? toYouTubeEmbedUrl(videoUrl, { muted }) : null
  ...
  // volume change to/from 0 remounts with the right `mute` param.
  if (embed) {
    mount(embed)
  } else {
    unmount()
