...
| `REDIS_URL` | No* | Full Redis URL. Backs three things: the BullMQ queue for the AI content generator, the Socket.IO adapter that lets several API instances share one realtime channel, and the lease that stops every instance running the same periodic job. Takes precedence over discrete `REDIS_HOST/PORT`. | *Required for the AI content generator (`OPENROUTER_API_KEY` set) and for running **more than one API instance**.* Prefer a full URL e.g. `rediss://:password@host:6379` and set `REDIS_TLS=true`. Locally `docker compose up -d redis`. A malformed value fails the boot rather than starting without Redis. |
| `REDIS_HOST` / `REDIS_PORT` / `REDIS_PASSWORD` / `REDIS_TLS` | No | Discrete Redis settings used when `REDIS_URL` is unset. | Optional; **no host default** — with `REDIS_URL` and `REDIS_HOST` both unset the API runs single-instance (in-memory sockets, every scheduled job unconditional) and logs that at boot. Port defaults `6379`. Set `REDIS_TLS=true` for managed Redis over TLS. |
...
Apps live in code as manifests (`APP_MANIFESTS` from `@signagewall/apps`). The catalog is a **mirror of that code** — nothing is added, renamed or deleted by hand. A **super-admin** (all `/admin/apps` routes are behind `SuperAdminGuard`) only decides which apps organizations may see:

1. **Sync on boot.** On every boot `AppsService.onModuleInit()` runs `syncManifestDefinitions()`. Every manifest that has no catalog entry is **created automatically** (unpublished, `isPublic:false`); every existing entry is kept in lockstep with its manifest **by slug** — `name`, `configSchema`, `version`, `runtimeKind`, `dataSource`, `overlay`, `iconSvg`, `color`. Shipping a new app, adding fields/sections, bumping the version or restyling an icon reaches the CMS and validation with a deploy, never a manual step. Copy (tagline/description/about) and categories are code + i18n, keyed by slug, and are not stored at all.
2. **Publish (make visible).** `PATCH /admin/apps/:id/visibility` with `{ isPublic: true }` — the one operator-owned setting, exposed in the CMS as the toggle in super-admin → Apps. Only public apps appear in an org's catalog via `findVisible()`. A newly deployed app stays invisible until someone flips it on.
3. **(Optional) Categorize.** A base taxonomy — *Information, Finance, Productivity, Data & Dashboards, Media, Social, Utilities* — is **seeded automatically on first boot** (idempotent). Manage the taxonomy via `/admin/app-categories` (POST/PATCH/DELETE, `SuperAdminGuard`); app→category membership itself is code-defined.
4. **(Optional) Beta grants.** `POST /admin/apps/:id/grants` entitles one named organization to a non-public app (and `DELETE .../grants/:organizationId` revokes it with the full uninstall cascade). This is how a design partner gets an app before it goes public.
5. **Org installs.** An organization sees public apps via `listCatalog` (`findVisible` + installed flag) and installs one (`orgAppsRepository.install`), enabling its users to create instances. `getCatalogApp` keeps a non-public app resolvable if the org already installed it, so unpublishing never locks an org out of existing instances.
6. **Uninstall cascades.** `uninstall` (`orgAppsRepository.uninstall` + `instancesService.removeAllForApp`) removes the org's instances of the app and their OAuth connections — no dangling playlist/screen references or orphaned connections. To retire an app entirely, remove its manifest from code and drop the row; there is no delete endpoint.
...
- **Steps:**
  ...
  5. Fill in **App Settings → Basic** completely — **Privacy Policy URL**, **Data Deletion**, **App Icon (1024×1024)** and **Category**. Meta disables Facebook Login on an app with any of these missing and shows the consent screen *"Facebook Login is currently unavailable for this app, since we are updating additional details for this app."* That message means the app is incomplete, not that our redirect URI or scopes are wrong — the request never reaches us.
  6. Register both **teardown callbacks** (the backend implements them; see below):
     - **Deauthorize Callback URL:** `https://<PUBLIC_API_URL>/api/v1/connections/meta/deauthorize`
     - **Data Deletion Request URL:** `https://<PUBLIC_API_URL>/api/v1/connections/meta/data-deletion`

     Meta accepts a plain **Data Deletion Instructions URL** instead of the callback, and `https://<MARKETING_URL>/data-deletion` (`/sr/brisanje-podataka`) is that page. Prefer the **callback**: the instructions page only tells someone what to do, while the callback actually revokes the tokens the moment they remove the app. Point the Privacy Policy field at `https://<MARKETING_URL>/privacy`.
- **Teardown callbacks:** Both are `@Public()` (Meta carries no bearer token) and authenticate the request by verifying the `signed_request` HMAC against **`META_CLIENT_SECRET`** — so with that secret unset they refuse rather than pretend to have deleted anything. Each disconnects every instance connected with that Facebook account, in **every** organization, matched on the app-scoped user id stored as `providerAccountId` when the account was connected. Deletion is synchronous, so the status page the receipt links to (`GET .../meta/data-deletion?code=…`) reports completion rather than a pending job. Connections made *before* this field existed carry no `providerAccountId` and cannot be matched — they lapse on token expiry instead.
