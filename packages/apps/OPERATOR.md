...
5. **(Optional) Categorize.** Manage categories via `/admin/app-categories` (POST/PATCH/DELETE, also `SuperAdminGuard`); attach `categoryIds` on create or via `PATCH /admin/apps/:id`. Presentation edits also go through `PATCH /admin/apps/:id`.
